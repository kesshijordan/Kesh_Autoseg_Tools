...
def doit(basedir, rootname, trkloc, lenthr, subsampN=12, save_snapshot=False, debug=False):
...
if __name__ == '__main__':
    ...
    lenthr = int(sys.argv[2])
    putloc = os.path.join(os.path.dirname(trkloc),'cci'+str(lenthr))
    if os.path.exists(trkloc):
        if not os.path.exists(putloc):
            os.mkdir(putloc)
        doit(putloc, os.path.basename(trkloc).replace('.trk','_cci'), trkloc, lenthr, 5, False)
